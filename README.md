...
> ✅ **2026-08-08 해소** — ④ 근거 검증이 `Task.VERIFY` 를 실제로 부르게 배선됐다.
> 2-gram이 바닥이고 LLM은 조이기만 하는 하이브리드 구조다.
> 다만 위 표의 *"조치가 발동하지 않는다"* 는 그대로 남아 있다.
...
원문을 그대로 적재할 수 있는 자료가 8건뿐이라 **경로 ②(사실 추출 + 문장화)가 기본**이다.
