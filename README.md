...
| | |
|---|---|
| 🔒 **트리아지 하향 금지 게이트** | `triage_level = max(rule, llm)` — **LLM은 등급을 낮출 수 없다.** 규칙이 낸 값은 확정이 아니라 **바닥**이다 (D-50) |
| **종 미확인 시 답변 금지** | 포유류 기준의 조류 적용은 치명적이다. 종이 없으면 검색 자체를 하지 않는다 |
| **근거 없으면 답하지 않는다** | 문장별 근거 검증 → 없는 문장 제거 → 재검색 1회 → 그래도 부족하면 **거절** |
| **원인을 모르면 좁히지 않는다** | 증상만 있거나 물질이 미상이면 검색으로 추정하지 않고 **되묻는다** (D-49) |
| **연락처는 코드가 뺀다** | 코퍼스가 전부 미국 자료라 답변에 미국 핫라인이 실린다. 출력 직전 문장 단위로 제거 (D-47) |
...
make test                                             # 안전 장치 회귀 (평가 지표 21건 포함)
...
**임계 경로는 그래프 노드 하나뿐이다.** **12개 함수**가 아직 비어 있고,
산출물 ③(구현 코드)과 ④(결과 보고서)가 **둘 다 여기 걸려 있다.**

**나머지는 전부 준비돼 있다** — 골든셋 52건 · 평가 하네스 · 계약 · 화면 · 코퍼스 888청크.
노드가 뚫리는 순간 `make eval` 이 채점까지 이어간다.

**밑바닥부터 쓰는 것은 아니다.** 게이트·규칙 테이블 조회·검색·중복 접기·연락처 차단·
프롬프트는 이미 구현되어 테스트가 붙어 있다. **절반(5개)은 LLM 을 부르지도 않는다.**
그리고 `EchoClient` 가 있어 **모델 없이 파이프라인 전체를 돌려 볼 수 있다** —
오늘 목표는 "노드 12개 구현"이 아니라 **"Echo 로 한 바퀴"** 다. → [`07 §1`](docs/07_작업배분.md)

| 담당 | 지금 | 읽을 것 |
|---|---|---|
| **이근준** | **그래프 노드 앞단 3** — `classify` · `slots` · `retrieve`. 최우선 | `pytest -m todo -k "classify or slot or retrieve"` · **`graph/nodes/__init__.py` 를 먼저 읽는다** |
| **오한빈** | **그래프 노드 뒷단 4** — `compute` · `generate` · `triage` · `verify` | `pytest -m todo` (남은 일 19건) |
| **이서은** | **골든셋 검수 27건**(G-026~G-052) → 파인튜닝 학습 데이터 | [`04a`](docs/04a_골든셋작성지침.md) |
| **권소라** | UI 프론트 — 세 상태(`answered`·`clarify`·`refused`)를 다 그린다 | `src/pettriage/app/web/index.html` |
| **오한빈** | 골든셋 8건(조류·고양이) · 평가 실행 · 결과 보고서 | [`04`](docs/04_테스트-평가계획.md) |

<details><summary>끝난 것</summary>

- 사실 표 **888행** · 45출처 — 검수 완료, 문장화 반전 3종 수정 (D-47~D-49)
- 인덱스 **888청크** 적재 · 검색 점검 양성 14/14 (04 §2.5.3~2.5.6)
- 계정·프로필 백엔드 흡수 (D-48) · 골든셋 27건 근거 대조 완료
- **골든셋 52/60건** — `case_type` 컬럼 신설 · `dose` 케이스 13건 (D-50 검증용)
- **평가 하네스** — `make eval` 로 지금 돈다. 노드를 기다리지 않고 `StubEngine` 으로 만들었다
- **D-50** 역치→등급 매핑 · 출처 상충 10배 룰 · **D-51** 식물 부위 단위는 환산하지 않는다
...
│   └── 08_코드검토/     구현물을 **실행해서** 검증한 기록. 회차별로 쌓인다
├── src/pettriage/
...
├── eval/
│   ├── goldenset/      골든셋 **52/60건** — `case_type` 별 배분        (WS4)
│   ├── harness/        `run_eval.py` · `metrics.py` — `make eval`
│   └── reports/        채점 결과 (JSON) — 커밋하지 않는다
├── scripts/            셸 진입점 (얇은 래퍼)
├── tests/              안전 장치 회귀 테스트
...
| 4 | [`06_결정기록`](docs/06_설계결정기록.md) | **결정 51건**의 맥락·대안·트레이드오프 |
| 5 | [`08_코드검토`](docs/08_코드검토/) | **만든 것이 실제로 그렇게 도는가** — 재현 코드와 함께 |
...
코드 검토: [`08`](docs/08_코드검토/) — 결함 36건 발견 → 전부 수정 (2026-08-02). 재현 코드 포함
도메인 선정 근거: [`도메인검토`](docs/도메인검토-반려동물-헬스케어.md) — 왜 반려동물 헬스케어였나 (15종 중 마지막)

> **"단점은 없나요?"** → 06의 트레이드오프 문장
> **"테스트가 통과하면 되는 것 아닌가요?"** → [`08_코드검토`](docs/08_코드검토/) — 테스트 269건이 전부 초록인 상태에서 안전 불변식 3개가 강제되지 않고 있었다
...
| 판정 | 건수 | 뜻 |
|---|---|---|
| 통과 | **8** | 원문 청크 적재 **가능** — 그러나 **적재하지 않았다** (아래) |
...
> **그 8건마저 넣지 않았다** (D-45). 두 경로를 함께 켜면 같은 사실이 두 번 적재되어
> top-k 를 잠식하고, 원문의 완화 표현과 사실 표의 등급이 ④ 근거 검증에서 모순으로 잡힌다.
> **지금 벡터DB의 888청크는 전부 우리가 쓴 한국어 문장이다** — 원문 청크 적재 **0건**.
...
- **유사도 점수로 거절 분기를 만들지 않는다** (D-46). 근거 있음(0.547~0.733)과
  없음(0.494~0.659) 분포가 **겹친다.** 임계값은 잡음 하한일 뿐이고,
  거절은 앞의 ①분류(범위밖)와 뒤의 ④검증(근거없음)이 만든다
- **규칙이 등급을 냈다고 LLM 판정을 건너뛰지 않는다** (D-50). 규칙은 *"역치를 넘었는가"* 만
  알고 *"얼마나 위험한가"* 는 모른다. 항상 `max(rule, llm)` 으로 간다
- **측정하지 않은 것을 `0` 으로 적지 않는다.** 분모가 0이면 `0.0%` 가 아니라 `—` 다 —
  "측정했는데 완벽했다"로 읽힌다. 평가 지표·점검 스크립트 모두 이 규칙을 따른다
- **판정 경로의 DB 조회에 pandas 를 쓰지 않는다** (D-52). `pd.read_sql` 은 `NULL` 을 `NaN` 으로
  바꾸고, `NaN` 은 `float` 이라 `is None` 검사를 통과한 뒤 **모든 역치 비교에서 `False`** 를
  돌려준다 — 체중을 모르는데 조용히 `MONITOR` 가 나간다. 집계(기간 리포트)에는 써도 된다
- **저장된 값을 판정에 쓰기 전에 확인을 받는다** (D-52). 프로필 체중은 낡는다 —
  되묻기를 없애는 것이 아니라 **확인으로 바꾼다**
