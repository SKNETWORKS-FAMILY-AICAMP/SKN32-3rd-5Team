...
**임계 경로는 그래프 노드다.** 11개 함수가 아직 비어 있고,
산출물 ③(구현 코드)과 ④(테스트 결과 보고서)가 **둘 다 여기 걸려 있다** —
파이프라인이 안 돌면 평가를 못 돌리고, 평가를 못 돌리면 보고서가 안 나온다.
...
오늘 목표는 "노드 11개 구현"이 아니라 **"Echo 로 한 바퀴"** 다. → [`07 §1`](docs/07_작업배분.md)

| 담당 | 지금 | 읽을 것 |
|---|---|---|
| **오한빈·이근준** | **그래프 노드 11개 함수** — 최우선 | `pytest -m todo` (남은 일 19건) |
| **이서은** | 골든셋 27 → 100건 · 파인튜닝 학습 데이터 | [`04a`](docs/04a_골든셋작성지침.md) |
| **권소라** | UI 프론트 — 세 상태(`answered`·`clarify`·`refused`)를 다 그린다 | `src/pettriage/app/web/index.html` |
| **오한빈** | 검수 · 평가 실행 · 결과 보고서 | [`04`](docs/04_테스트-평가계획.md) |
...
| 4 | [`06_결정기록`](docs/06_설계결정기록.md) | **결정 44건**의 맥락·대안·트레이드오프 |
