...
| 담당 | 맡은 것 |
|---|---|
| **오한빈** (팀장) | **전체 시스템·파이프라인 기획, 설계, 구축, 검증** · 데이터 전처리 · 골든셋 일부 작성과 검토 · 설계 결정 기록 · **프로젝트 총괄** |
| **이근준** | **백엔드** — UI와 이어지는 DB 설계, 반려동물 프로필, 계정·다이어리 API<br>**LangGraph 파이프라인 구축** (팀장과 공동) |
| **권소라** | **UI 화면 전체 구성 및 검토** · 발표 자료(PPT) 작성 |
| **이서은** | **골든셋 작성 및 검토** · **파인튜닝 담당 및 검토**(Qwen3-4B QLoRA) · **원 기획안 제시** |

> **LangGraph 파이프라인은 오한빈·이근준 공동 구축이다.** 그래프 조립과 노드 배선을
> 함께 했고, 그 과정에서 나온 결정(D-64 랭그래프 도입 · D-90 라우터 단독 상한 판단)이
> `docs/06` 에 남아 있다.
