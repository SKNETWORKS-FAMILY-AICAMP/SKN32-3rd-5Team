...
#: 🔴 **질의 그래프에서 실제로 모델을 부르는 태스크는 셋뿐이다.**
#:
#: 05 §4 는 다섯을 적어 두었지만 —
#:
#:   · `Task.VERIFY` 는 **어디서도 `client.run` 으로 불리지 않는다.**
#:     `verify_grounding` 이 2-gram 문자 일치율로 판정한다 (2026-08-03 확인).
#:     부르지 않으니 실패할 일도 없고, 그래서 폴백 집계에서 **성공한 것처럼 보였다.**
#:     리포트의 *"5태스크 전부 모델 100%"* 가 그 거짓 신호다.
